#include <gui/piecharttogglefullsemi_screen/PieChartToggleFullSemiView.hpp>
#include <touchgfx/Math3D.hpp>

PieChartToggleFullSemiView::PieChartToggleFullSemiView():
    pieChartClickedCallback(this, &PieChartToggleFullSemiView::pieChartClickHandler),
    ...
    radiusAnimation(0),
	angleStartAnimation(0), // Full circle
    angle1Animation(0),
    angle2Animation(0),
    angle3Animation(0),
    angle4Animation(0),
    ...
    angle5Target(0),
    angle6Target(0),
	isFullCircle(true) // Full circle
{

}

void PieChartToggleFullSemiView::setupScreen()
{
    PieChartToggleFullSemiViewBase::setupScreen();
    ...
    // Use dynamic angle range
    int angleRange = getAngleRange();  // 360 or 180
    angle1Target = (serie1Value * angleRange) / 100;
    angle2Target = (serie2Value * angleRange) / 100;
    angle3Target = (serie3Value * angleRange) / 100;
    angle4Target = (serie4Value * angleRange) / 100;
    angle5Target = (serie5Value * angleRange) / 100;
    angle6Target = (serie6Value * angleRange) / 100;
...
void PieChartToggleFullSemiView::tearDownScreen()
{
    PieChartToggleFullSemiViewBase::tearDownScreen();
}

void PieChartToggleFullSemiView::handleTickEvent()
{
...
void PieChartToggleFullSemiView::updateQuarterData()
{
...
void PieChartToggleFullSemiView::updatePieChart(int s1, int s2, int s3, int s4, int s5, int s6)
{
    ...
    // Use dynamic angles
     const int START_ANGLE = getStartAngle();      // 0 or 270
     const int ANGLE_RANGE = getAngleRange();      // 360 or 180

     // Calculate individual slice widths
     int angle1 = (serie1Value * ANGLE_RANGE) / 100;
     int angle2 = (serie2Value * ANGLE_RANGE) / 100;
     int angle3 = (serie3Value * ANGLE_RANGE) / 100;
     int angle4 = (serie4Value * ANGLE_RANGE) / 100;
     int angle5 = (serie5Value * ANGLE_RANGE) / 100;
     int angle6 = (serie6Value * ANGLE_RANGE) / 100;
...
void PieChartToggleFullSemiView::pieChartClickHandler(const touchgfx::Container& src, const touchgfx::ClickEvent& evt)
{
    ...
    const int START_ANGLE = getStartAngle();
    const int ANGLE_RANGE = getAngleRange();
    // Calculate individual slice widths
    int angle1 = (serie1Value * ANGLE_RANGE) / 100;
    int angle2 = (serie2Value * ANGLE_RANGE) / 100;
    int angle3 = (serie3Value * ANGLE_RANGE) / 100;
...
int PieChartToggleFullSemiView::getClickedSlice(int16_t clickX, int16_t clickY)
{
    const int16_t centerX = 120;
    const int16_t centerY = 120;
...
//    // Convert 270°-360° range to 270°-360°, and 0°-90° range to 360°-450°
    ...
    // Dynamic angle validation
    const int START_ANGLE = getStartAngle();
    const int ANGLE_RANGE = getAngleRange();

    if (isFullCircle)
    {
        // Full circle: all angles are valid (0° → 360°)
        // No filtering needed
    }
    else
    {
        // Semicircle (270° → 90°): reject 90° < angle < 270°
        if (angleInDegrees > 90 && angleInDegrees < 270)
        {
            return 0;  // Outside semicircle range
        }
...
void PieChartToggleFullSemiView::moveSlice(touchgfx::Circle& slice, int arcStart, int arcEnd, bool moveOut)
{
    if (moveOut)
    {
...
void PieChartToggleFullSemiView::toggleAllSlices()
{
	const int START_ANGLE = getStartAngle();   // 0 or 270
	const int ANGLE_RANGE = getAngleRange();   // 360 or 180
...
void PieChartToggleFullSemiView::drawPieChartAnimation()
{
    ...
    const int ANIM_START = getStartAngle();
    const int ANIM_END = getEndAngle();
    ...
    {
        // Set final arcs using individual slice widths
        const int START_ANGLE = getStartAngle();
    	const int ANGLE_RANGE = getAngleRange();
...
int PieChartToggleFullSemiView::getStartAngle() const
{
    return isFullCircle ? 0 : 270;
}

int PieChartToggleFullSemiView::getAngleRange() const
{
    return isFullCircle ? 360 : 180;
}

int PieChartToggleFullSemiView::getEndAngle() const
{
    return isFullCircle ? 360 : 450;  // 450 wraps to 90° for semicircle
}

void PieChartToggleFullSemiView::toggleCircleMode()
{
    // Toggle the mode
    isFullCircle = !isFullCircle;

    // Collapse all slices before switching (cleaner transition)
    if (allSlicesExpanded)
    {
        toggleAllSlices();
    }

    // Reset animation start angle for new mode
    angleStartAnimation = getStartAngle();

    // Redraw chart with current values
    updatePieChart(serie1Value, serie2Value, serie3Value,
                   serie4Value, serie5Value, serie6Value);
}
